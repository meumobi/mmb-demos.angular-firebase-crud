...
describe('ItemMockService', () => {
  ...
  it('should return an array with one new item', (done: DoneFn) => {
    const countItemsOnInit = itemMockService.$data().length;
    itemMockService.create({
      title: 'lorem ipsum',
      description:
  ...
  it('should return a shorter array, one item left', (done: DoneFn) => {
    const countItemsOnInit = itemMockService.$data().length;
    itemMockService.delete('123');
    expect(itemMockService.$data().length == countItemsOnInit - 1).toBeTrue();
    done();
  });
  it('should return an item by id', (done: DoneFn) => {
